...
import { DEFAULT_LANGUAGE, getPreferredLanguage, setLanguageCode } from './constants/lang.js';
...
export class Sim {
	...
	private titanPhase: number = OtherConstants.getPreferredTitanPhase();
	...
	private language: string = getPreferredLanguage();
	...
	readonly titanPhaseChangeEmitter = new TypedEvent<void>();
	...
	getTitanPhase(): number {
		return this.titanPhase;
	}
	setTitanPhase(eventID: EventID, newPhase: number) {
		if (newPhase != this.titanPhase && newPhase > 0) {
			this.titanPhase = newPhase;
			OtherConstants.setPreferredTitanPhase(newPhase);
			this.titanPhaseChangeEmitter.emit(eventID);
		}
	}
	...
	setLanguage(eventID: EventID, newLanguage: string) {
		newLanguage = newLanguage || DEFAULT_LANGUAGE;
	...
	fromProto(eventID: EventID, proto: SimSettingsProto) {
		TypedEvent.freezeAllAndDo(() => {
			...
			this.setLanguage(eventID, getPreferredLanguage());
