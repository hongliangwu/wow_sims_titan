...
export abstract class SimUI extends Component {
	...
	addTab(title: string, cssClass: string, innerHTML: string) {
		...
		this.simHeader.addTab(t(title), contentId);
	...
	private addKnownIssues(config: SimUIConfig) {
		...
		if (config.knownIssues && config.knownIssues.length) {
			config.knownIssues.forEach(issue => this.simHeader.addKnownIssue(t(issue)));
		}
		globalKnownIssues.forEach(issue => this.simHeader.addKnownIssue(t(issue)));
	}
	...
	async handleCrash(error: any): Promise<void> {
		...
		if (
			window.confirm(t('Simulation Failure:\n') + errorStr + t('\nPress Ok to file crash report'))
		) {
			...
			fetch(
				...
				.catch(fetchErr => {
					alert(t('Failed to file report... try again another time:') + fetchErr);
				});
		}
	}
	...
	constructor(parent: HTMLElement, link: string) {
		...
		this.body.innerHTML = `
			<div class="sim-crash-report">
				<h3 class="sim-crash-report-header">${t('Please append the following complete link to the issue you just created. This will simplify debugging the issue.')}</h3>
