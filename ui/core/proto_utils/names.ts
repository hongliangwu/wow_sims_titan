...
import { localizeMap, localizeRecord, t } from '../i18n.js';

export const armorTypeNames: Map<ArmorType, string> = localizeMap(new Map([
...
export const weaponTypeNames: Map<WeaponType, string> = localizeMap(new Map([
...
export const rangedWeaponTypeNames: Map<RangedWeaponType, string> = localizeMap(new Map([
...
const raceNamesEn: Map<Race, string> = new Map([
...
export const raceNames: Map<Race, string> = localizeMap(raceNamesEn);

export function nameToRace(name: string): Race {
	const normalized = name.toLowerCase().replaceAll(' ', '');
	for (const map of [raceNamesEn, raceNames]) {
...
const classNamesEn: Map<Class, string> = new Map([
...
export const classNames: Map<Class, string> = localizeMap(classNamesEn);
export { classNamesEn };

export function nameToClass(name: string): Class {
	const lower = name.toLowerCase().replace(/\s+/g, '');
	for (const map of [classNamesEn, classNames]) {
		for (const [key, value] of map) {
...
const professionNamesEn: Map<Profession, string> = new Map([
...
export const professionNames: Map<Profession, string> = localizeMap(professionNamesEn);

export function nameToProfession(name: string): Profession {
	const lower = name.toLowerCase();
	for (const map of [professionNamesEn, professionNames]) {
		for (const [key, value] of map) {
...
export const statNames: Map<Stat, string> = localizeMap(new Map([
...
export const pseudoStatNames: Map<PseudoStat, string> = localizeMap(new Map([
...
export function getClassStatName(stat: Stat, playerClass: Class): string {
	if (playerClass == Class.ClassHunter) {
		if (stat == Stat.StatMeleeHit) return t('Ranged Hit');
		if (stat == Stat.StatMeleeCrit) return t('Ranged Crit');
		if (stat == Stat.StatMeleeHaste) return t('Ranged Haste');
	}
	return statNames.get(stat) || t('Unknown');
}

export const slotNames: Map<ItemSlot, string> = localizeMap(new Map([
...
const resourceNamesEn: Map<ResourceType, string> = new Map([
...
export const resourceNames: Map<ResourceType, string> = localizeMap(resourceNamesEn);
...
export function stringToResourceType(str: string): ResourceType {
	for (const map of [resourceNamesEn, resourceNames]) {
...
export const sourceNames: Map<SourceFilterOption, string> = localizeMap(new Map([
...
export const raidNames: Map<RaidFilterOption, string> = localizeMap(new Map([
...
export const difficultyNames: Map<DungeonDifficulty, string> = localizeMap(new Map([
...
export const REP_LEVEL_NAMES: Record<RepLevel, string> = localizeRecord({
...
export const REP_FACTION_NAMES: Record<RepFaction, string> = localizeRecord({
