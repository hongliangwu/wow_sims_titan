...
import { isTitanCustomItemId } from './item_tooltip.js';
...
export class ActionId {
	...
	static fromItem(item: Item): ActionId {
		const iconUrl = item.icon ? ActionId.makeIconUrl(item.icon) : '';
		return new ActionId(item.id, 0, OtherAction.OtherActionNone, 0, item.name || '', item.name || '', iconUrl);
	}
