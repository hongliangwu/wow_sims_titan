...
export class SavedDataManager<ModObject, T> extends Component {
	...
	private makeSavedData(config: SavedDataConfig<ModObject, T>): SavedData<ModObject, T> {
		...
		if (!config.isPreset) {
			...
			const tooltip = Tooltip.getOrCreateInstance(deleteButton, {title:`${t('Delete saved')} ${t(this.config.label)}`});
	...
	private buildCreateContainer(): HTMLElement {
		...
		savedDataCreateFragment.innerHTML = `
			<div class="saved-data-create-container">
				<label class="form-label">${t(this.config.label)} ${t('Name')}</label>
				<input class="saved-data-save-input form-control" type="text" placeholder="${t('Name')}">
				<button class="saved-data-save-button btn btn-primary">${t('Save')} ${t(this.config.label)}</button>
			</div>
		`;
		...
		saveButton.addEventListener('click', event => {
			...
			if (!newName) {
				alert(`${t('Choose a label for your saved')} ${t(this.config.label)}!`);
				return;
			}

			if (newName in this.presets) {
				alert(`${t(this.config.label)} ${newName} ${t('already exists.')}`);
				return;
			}
