...
export function makeTitanPhaseSelector(parent: HTMLElement, sim: Sim): EnumPicker<Sim> {
	return new EnumPicker<Sim>(parent, sim, {
		extraCssClasses: ['phase-selector', 'titan-phase-selector'],
		label: 'Titan Phase',
		inline: true,
		values: [
			{ name: 'P1', value: 1 },
		...
		changedEvent: (sim: Sim) => sim.titanPhaseChangeEmitter,
		getValue: (sim: Sim) => sim.getTitanPhase(),
		setValue: (eventID: EventID, sim: Sim, newValue: number) => {
			sim.setTitanPhase(eventID, newValue);
		},
	});
}
