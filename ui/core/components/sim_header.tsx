...
export class SimHeader extends Component {
	...
	private addBugReportLink() {
		this.addToolbarLink({
			...
			tooltip: t('Report a bug or<br>Request a feature'),
		});
	}
	...
	private addDownloadBinaryLink() {
		...
		if (document.location.href.includes('localhost')) {
					...
					.then(versionInfo => {
						if (versionInfo.outdated == 2) {
							this.addToolbarLink({
								...
								parent: parent,
								icon: icon,
								tooltip: t('Newer version of simulator available for download'),
			...
			this.addToolbarLink({
				...
				parent: parent,
				icon: icon,
				tooltip: t('Download simulator for faster simulating'),
				classes: 'downbin',
			});
		}
	}
