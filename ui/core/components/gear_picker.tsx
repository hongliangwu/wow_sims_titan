...
const createTitanLabel = (phase?: number) => {
	return <span className="titan-item-badge">[{titanOriginLabel(phase)}]</span>;
};
...
const applyItemNameLabels = (nameElem: HTMLElement, item: { id: number; heroic?: boolean; phase?: number }) => {
	if (isTitanCustomItemId(item.id)) {
		...
		nameElem.appendChild(createPhaseLabel(item.phase));
	}
};
...
export class ItemRenderer extends Component {
	...
	clear() {
		clearLocalItemTooltip(this.nameElem);
		clearLocalItemTooltip(this.iconElem);
	...
	update(newItem: EquippedItem) {
		...
		applyItemNameLabels(this.nameElem, newItem.item);
		...
		newItem
			...
			.then(filledId => {
				...
				if (getLanguageCode() && filledId.name) {
					this.nameElem.textContent = filledId.name;
					applyItemNameLabels(this.nameElem, newItem.item);
				}
			});
...
export class IconItemSwapPicker extends Component {
	...
	update(newItem: EquippedItem | null) {
		...
		this.socketsContainerElem.innerText = '';
		clearLocalItemTooltip(this.iconAnchor);
...
export class SelectorModal extends BaseModal {
	...
	private addTab<T>(
		...
		if (setTabContent) {
			...
			tabAnchor.value!.textContent = t(label);
		}
		...
		this.player.sim.titanPhaseChangeEmitter.on(applyFilter);
		...
		this.addOnDisposeCallback(() => {
			...
			this.player.sim.titanPhaseChangeEmitter.off(applyFilter);
...
export class ItemList<T> {
	...
	constructor(
		...
		this.tabContent = (
			<div id={tabContentId} className={`selector-modal-tab-pane tab-pane fade ${selected ? 'active show' : ''}`}>
				<div className="selector-modal-filters">
					<input className="selector-modal-search form-control" type="text" placeholder={t('Search...')} />
					{label == 'Items' && <button className="selector-modal-filters-button btn btn-primary">{t('Filters')}</button>}
					<div className="selector-modal-titan-phase-selector"></div>
					...
					<button className="selector-modal-simall-button btn btn-warning">{t('Add to Batch Sim')}</button>
					<button className="selector-modal-remove-button btn btn-danger">{t('Unequip Item')}</button>
				</div>
		...
		if (label == 'Items') {
			const titanPhasePicker = makeTitanPhaseSelector(
				this.tabContent.getElementsByClassName('selector-modal-titan-phase-selector')[0] as HTMLElement,
				player.sim,
			);
			titanPhasePicker.changeEmitter.on(() => this.applyFilters());
		} else {
			(this.tabContent.getElementsByClassName('selector-modal-titan-phase-selector')[0] as HTMLElement).style.display = 'none';
		}
		...
		if (label.startsWith('Enchants')) {
			removeButton.textContent = t('Remove Enchant');
		} else if (label.startsWith('Gem')) {
			removeButton.textContent = t('Remove Gem');
		}
		...
		itemIdxs = itemIdxs.filter(i => {
			...
			if (this.searchInput.value.length > 0) {
				const searchQuery = this.searchInput.value
					.toLowerCase()
					.replaceAll(/[^a-zA-Z0-9\u4e00-\u9fff\s]/g, '')
					.split(' ')
					.filter(v => v.length > 0);
				const name = listItemData.name.toLowerCase().replaceAll(/[^a-zA-Z0-9\u4e00-\u9fff\s]/g, '');
	...
	private createItemElem(item: ItemDataWithIdx<T>): JSX.Element {
		...
		itemData.actionId.fill().then(filledId => {
			...
			if (getLanguageCode() && filledId.name && nameElem.value) {
				...
				nameElem.value.textContent = filledId.name;
				applyItemNameLabels(nameElem.value, {
					id: itemData.id,
					heroic: itemData.heroic,
	...
	private getSourceInfo(item: Item, sim: Sim): JSX.Element {
		...
		if (!item.sources || item.sources.length == 0) {
			if (isTitanCustomItemId(item.id)) {
				return <small className="titan-item-badge">{titanOriginLabel(item.phase)}</small>;
			}
			return <></>;
		}
