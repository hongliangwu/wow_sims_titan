...
export class RaidSimResultsManager {
	...
	setSimProgress(progress: ProgressMetrics) {
		this.simUI.resultsViewer.setContent(`
			<div class="results-sim">
				...
				<div class="">
					${progress.presimRunning ? t('presimulations running') : `${progress.completedIterations} / ${progress.totalIterations}<br>${t('iterations complete')}`}
				</div>
			</div>
		`);
	}
    ...
    `);
		...
		setResultTooltip('results-sim-dps', t('Damage Per Second'));
		setResultTooltip('results-sim-dpasp', t('Demonic Pact Average Spell Power'));
		setResultTooltip('results-sim-tto', t('Time To OOM'));
		setResultTooltip('results-sim-hps', t('Healing+Shielding Per Second, including overhealing.'));
		setResultTooltip('results-sim-tps', t('Threat Per Second'));
		setResultTooltip('results-sim-dtps', t('Damage Taken Per Second'));
		setResultTooltip('results-sim-tmi', t('TMI_TOOLTIP'));
		setResultTooltip('results-sim-cod', t('COD_TOOLTIP'));
		...
		Tooltip.getOrCreateInstance(simReferenceSetButton, {title: t('Use as reference')});
		...
		tippy(simReferenceSwapButton, {
			'content': t('Swap reference with current'),
		...
		resultsFragment.innerHTML = `
			<div class="results-metric ${args.classes}">
				...
				<div class="results-reference hide">
					<span class="results-reference-diff"></span> ${t('vs reference')}
				</div>
			</div>
		`;
