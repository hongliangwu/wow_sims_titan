...
import { DEFAULT_LANGUAGE, wowheadSupportedLanguages } from '../constants/lang.js';
...
export class SettingsMenu extends BaseModal {
	...
	constructor(parent: HTMLElement, simUI: SimUI) {
		...
		this.body.innerHTML = `
			<div class="picker-group">
				<div class="fixed-rng-seed-container">
					...
					<div class="form-text">
						<span>${t('Last used RNG seed:')}</span>&nbsp;<span class="last-used-rng-seed">0</span>
					</div>
				</div>
		...
		Tooltip.getOrCreateInstance(restoreDefaultsButton, {
			title: t('Restores all default settings (gear, consumes, buffs, talents, EP weights, etc). Saved settings are preserved.')
		});
		...
		const defaultLang = langs.indexOf(DEFAULT_LANGUAGE);
		const languagePicker = new EnumPicker(language, this.simUI.sim, {
			label: 'Language',
			labelTooltip: t('Controls the language for Wowhead tooltips.'),
			...
			setValue: (eventID: EventID, sim: Sim, newValue: number) => {
				sim.setLanguage(eventID, langs[newValue] || DEFAULT_LANGUAGE);
			},
		});
