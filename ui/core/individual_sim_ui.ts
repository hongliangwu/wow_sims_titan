...
export abstract class IndividualSimUI<SpecType extends Spec> extends SimUI {
	...
	fromProto(
		...
		TypedEvent.freezeAllAndDo(() => {
			...
			// Apply language before gear/UI so Wowhead tooltips and localized names
			// are requested in the selected locale on first render.
			if (loadCategory(SimSettingCategories.UISettings) && settings.settings) {
